...
apt-get install vim curl zsh git htop sudo tmux net-tools -y
...
sed -i 's/^ExecStart=.*/ExecStart=-\/sbin\/agetty\ --noclear\ --autologin\ root\ --keep-baud\ tty%I\ 115200,38400,9600\ $TERM/' /lib/systemd/system/container-getty@.service
...
git config --global user.name "Daniel Blittschau Homelab"
...
sed -i 's/^ZSH_THEME=.*/ZSH_THEME="dblitt99"/' $HOME/.zshrc

# Disable oh-my-zsh auto updates
sed -i '1iDISABLE_AUTO_UPDATE=true' $HOME/.zshrc
